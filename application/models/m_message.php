...
class M_message extends CI_Model{
	...
	function get_outbox(){
		$query_string = "Select ID, DATE_FORMAT(SendingDateTime, '%d-%m-%Y') AS SendingDate, DATE_FORMAT(SendingDateTime, '%T') AS SendingTime, DestinationNumber, TextDecoded from outbox ORDER BY SendingDateTime DESC";
		$query = $this->db->query($query_string);
		if ($query->num_rows() > 0)
			return $query->result_array();
		else
